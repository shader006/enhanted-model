...
class DeterministicPatchDataLoader(DataLoaderMultiProcess):
    ...
    def generate_train_batch(self):
        selected_keys = [
            self.keys[(self._next_index + i) % len(self.keys)]
            for i in range(self.batch_size)
        ]
        self._next_index = (self._next_index + self.batch_size) % len(self.keys)
        ...
        for j, key in enumerate(selected_keys):
            ...
            shape = data.shape[1:]
            dim = len(shape)
            bbox_lbs, bbox_ubs = self.get_center_bbox(shape)
            valid_bbox_lbs = [max(0, bbox_lbs[i]) for i in range(dim)]
            valid_bbox_ubs = [min(shape[i], bbox_ubs[i]) for i in range(dim)]
    ...
    def get_center_bbox(self, data_shape):
        need_to_pad = self.need_to_pad.copy()
        dim = len(data_shape)
        for d in range(dim):
            if need_to_pad[d] + data_shape[d] < self.patch_size[d]:
        ...
        lbs = [-need_to_pad[i] // 2 for i in range(dim)]
        ubs = [data_shape[i] + need_to_pad[i] // 2 + need_to_pad[i] % 2 - self.patch_size[i] for i in range(dim)]
        bbox_lbs = [(lbs[i] + ubs[i]) // 2 for i in range(dim)]
        bbox_ubs = [bbox_lbs[i] + self.patch_size[i] for i in range(dim)]
        return bbox_lbs, bbox_ubs
