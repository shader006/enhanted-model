...
class DiceFocalTverskyLoss(nn.Module):
    ...
    def forward(self, logits, target):
        ...
        focal_tversky_loss = torch.pow(1.0 - tversky_score, 1.0 / self.gamma).mean()
...
class BraTSTrainer(Trainer):
    ...
    def apply_modality_dropout(self, image):
        if not settings.SEGMAMBA_MODALITY_DROPOUT_ENABLED:
            return image
        if settings.SEGMAMBA_MODALITY_DROPOUT_PROB <= 0:
            return image
        if torch.rand(1, device=image.device).item() >= settings.SEGMAMBA_MODALITY_DROPOUT_PROB:
            return image

        channels = image.shape[1]
        max_drop = min(settings.SEGMAMBA_MODALITY_DROPOUT_MAX_CHANNELS, channels - 1)
        if max_drop <= 0:
            return image

        num_drop = int(torch.randint(1, max_drop + 1, (1,), device=image.device).item())
        drop_channels = torch.randperm(channels, device=image.device)[:num_drop]
        image = image.clone()
        image[:, drop_channels] = 0
        return image

    def training_step(self, batch):
        image, label = self.get_input(batch)
        image = self.apply_modality_dropout(image)
