...
try:
    ...
    causal_conv1d_fwd_function, causal_conv1d_bwd_function = None, None
import causal_conv1d_cuda
import selective_scan_cuda


def _causal_conv1d_fwd(x, conv1d_weight, conv1d_bias, activation):
    if causal_conv1d_fwd_function is not None:
        return causal_conv1d_fwd_function(
            x, conv1d_weight, conv1d_bias, None, None, None, activation
        )
    try:
        return causal_conv1d_cuda.causal_conv1d_fwd(
            x, conv1d_weight, conv1d_bias, activation
        )
    except TypeError:
        out = torch.empty_like(x)
        causal_conv1d_cuda.causal_conv1d_fwd(
            x, conv1d_weight, conv1d_bias, None, None, out, None, activation
        )
        return out


def _causal_conv1d_bwd(x, conv1d_weight, conv1d_bias, dconv1d_out, dx, activation):
    if causal_conv1d_bwd_function is not None:
        dx_out, dconv1d_weight, dconv1d_bias, _ = causal_conv1d_bwd_function(
            x, conv1d_weight, conv1d_bias, dconv1d_out,
            None, None, None, dx, False, activation
        )
        return dx_out, dconv1d_weight, dconv1d_bias
    try:
        return causal_conv1d_cuda.causal_conv1d_bwd(
            x, conv1d_weight, conv1d_bias, dconv1d_out, dx, activation
        )
    except TypeError:
        dx_out = dx if dx is not None else torch.empty_like(x)
        dconv1d_weight = torch.empty_like(conv1d_weight)
        dconv1d_bias = torch.empty_like(conv1d_bias) if conv1d_bias is not None else None
        causal_conv1d_cuda.causal_conv1d_bwd(
            x, conv1d_weight, conv1d_bias, dconv1d_out,
            None, None, None, dx_out, dconv1d_weight, dconv1d_bias, None, activation
        )
        return dx_out, dconv1d_weight, dconv1d_bias
...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        conv1d_out = _causal_conv1d_fwd(x, conv1d_weight, conv1d_bias, True)
    ...
    def backward(ctx, dout):
        ...
        dx, dconv1d_weight, dconv1d_bias = _causal_conv1d_bwd(
