...
def window_reverse(windows, window_size, dims):
    ...
    if len(dims) == 4:
        ...
        x = x.permute(0, 1, 4, 2, 5, 3, 6, 7).contiguous().view(b, d, h, w,
                                                                -1)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        if len(x_shape) == 5:
            b, d, h, w, c = x.shape
            window_size, shift_size = get_window_size((d, h, w), self.window_size, self.shift_size)  # 窗大小、移位步长
        ...
        attn_windows = attn_windows.view(-1, *(
                window_size + (c,)))
        shifted_x = window_reverse(attn_windows, window_size,
                                   dims)
        if any(i > 0 for i in shift_size):
            if len(x_shape) == 5:
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, normalize=True):
        x0 = self.patch_embed(x)  # Patch Partition + Linear Embed
        x0 = self.pos_drop(x0)
        x0_out = self.proj_out(x0, normalize)
        x1 = self.layers1[0](x0.contiguous())  # stage 1
        x1_out = self.proj_out(x1, normalize)
        x2 = self.layers2[0](x1.contiguous())  # stage 2
        x2_out = self.proj_out(x2, normalize)
        x3 = self.layers3[0](x2.contiguous())  # stage 3
        x3_out = self.proj_out(x3, normalize)
        x4 = self.layers4[0](x3.contiguous())  # stage 4
...
class UnetrUpBlockWithAttention(nn.Module):
    ...
    def __init__(
        ...
        self.skipatten = SkipAttentionBlock(F_g=out_channels, F_l=out_channels, F_int=in_channels)
