...
def unfold2d(
    ...
    *,
    ...
    """
    See :math:`~torch.nn.functional.unfold`
    """
...
def filter_dilated_rows(
    ...
    # for dim in range(kernel_rank):
    #     # Expanding indices to match the input tensor dimensions
    #     expand_shape = [-1 if i == dim else 1 for i in range(kernel_rank)]
    #     indices = indices_to_keep[dim].view(*expand_shape)
    #
    #     # Repeating indices to match the input tensor shape for proper indexing
    #     repeat_shape = [kernel_size[i] if i == dim else 1 for i in range(kernel_rank)]
    #     indices = indices.repeat(*repeat_shape)
    #
    #     # Slicing the tensor for each dimension
    #     dim_index = len(tensor.shape) - kernel_rank + dim
    #     tensor = tensor.index_select(dim_index, indices)
    ...
    # ]
    #
    # indices_to_keep = torch.tensor(indices_to_keep).to(tensor.device)
    #
    # # tensor_np = tensor.cpu().detach().numpy()
    #
    # axis_offset = len(tensor.shape) - kernel_rank
    #
    # for dim in range(kernel_rank):
    #     axis = torch.tensor(axis_offset + dim).to(tensor.device)
    #     tensor = torch.index_select(tensor, dim=axis, index=indices_to_keep[dim])
    #
    # return tensor
