...
from SRUNet.wavelet_downsampling import WaveletDownsampleArray
from nnunetv2.training.data_augmentation.custom_transforms.deep_supervision_donwsampling import DownsampleArray
...
class nnUNetWaveletPredictor(object):
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor) \
        ...
        if self.use_wavelets:
            downsample_array = WaveletDownsampleArray(ds_scales=self.ds_scales, mode=self.subband_mode, wavelet=self.wavelet_function)
        elif self.input_downsample:
            downsample_array = DownsampleArray(ds_scales=self.ds_scales)
        if self.is_ddp:
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                for sl in tqdm(slicers, disable=not self.allow_tqdm):
                    workon = data[sl][None]
                    workon = downsample_array(workon)
                    if isinstance(workon, list):
                        if self.is_ddp:
                            workon = [i.to(self.network.device, non_blocking=True) for i in workon]
                        else:
