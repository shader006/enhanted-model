...
def test(label_p, infer_p):
    label_list = sorted(glob.glob(os.path.join(label_p,'*nii.gz')))
    infer_list = sorted(glob.glob(os.path.join(infer_p, '*nii.gz')))
