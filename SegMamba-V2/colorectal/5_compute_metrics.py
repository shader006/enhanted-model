...
if __name__ == "__main__":
    ...
    raw_data_dir = "/data/xingzhaohu/colorectal/data/raw_data/random2500/ROI"
    train_ds, val_ds, test_ds = get_train_val_test_loader_from_train(data_dir)
    print(len(test_ds))
    all_results = np.zeros((499,2))

    ind = 0
    for batch in tqdm(test_ds, total=len(test_ds)):
        properties = batch["properties"]
        case_name = properties["name"]
        gt_itk = os.path.join(raw_data_dir, f"{case_name}.nii.gz")
        voxel_spacing = [1, 1, 1]
        ...
        m = cal_metric(gt_array, pred_array, voxel_spacing)
        print(f"m is {m}")
        all_results[ind, ...] = m
