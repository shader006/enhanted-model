...
class MSAM(torch.optim.Optimizer):
    ...
    def step(self):
        ...
        for i, param in enumerate(params_with_grad):
            ...
            if buf is not None and (buf.device != param.device or buf.dtype != param.dtype):
                # Optimizer may be created before model.to(device); align state buffer on first step.
                buf = buf.to(device=param.device, dtype=param.dtype)
                momentum_buffer_list[i] = buf

            param.add_(buf, alpha=rho * float(group["inverse_norm_buffer"]))
        ...
        # Keep as Python float so alpha in in-place ops never carries a tensor device.
        group["inverse_norm_buffer"] = 1.0 / (float(ascent_norm.item()) + 1e-12)

        # second loop: 
        # p_{t+1} = p_{t+1}'' - rho*v_{t+1}/||v_{t+1}|| <- ascending again
        for i, param in enumerate(params_with_grad):
            param.sub_(momentum_buffer_list[i], alpha=rho * float(group["inverse_norm_buffer"]))
            ...
            for p in group['params']:
                if "momentum_buffer" in self.state[p]:
                    p.sub_(
                        ...
                        alpha=group["rho"] * float(group["inverse_norm_buffer"]),
                    )
