...
class Trainer:
    def __init__(self, env_type,
        ...
        self.val_number = None
        self.augmentation = True
        self.train_process = train_process
        self.val_process = 6
        self.augmenter_backend = "nondet-multiprocess"
        self.augmenter_seeds = None
        self.seed = 42
        self.enable_tensorboard = True
        self.amp_enabled = self.device != "cpu"
    ...
    def get_multi_processor_loader(self, train_ds, val_ds):
        from .augment.multi_processor import create_limited_len_augmenter
        ...
        print(f"augmenter backend: {self.augmenter_backend}")
        data_generator = create_limited_len_augmenter(
            mode=self.augmenter_backend,
            my_imaginary_length=self.num_step_per_epoch,
    ...
    def validate(self):
        ...
        if self.global_step % self.val_every == 0 \
            ...
            for i in tqdm(range(len(self.val_loader)), total=len(self.val_loader)):
                ...
                with torch.no_grad():
                    with torch.autocast("cuda", dtype=self.amp_dtype, enabled=self.amp_enabled) if (self.ddp or 'cuda' in self.device) else dummy_context():
                        val_out = self.validation_step(batch)
                        assert val_out is not None 
            ...
            if self.ddp:
                ...
                val_outputs_merge = []
                for i in range(len(outputs_split)):
                    val_outputs = torch.as_tensor(np.asarray(outputs_split[i]))
    ...
    def train(self,
        ...
        if callable(scheduler_builder):
            self.scheduler = scheduler_builder(
                self.optimizer,
                num_training_steps=self.max_steps,
                num_epochs=self.max_epochs,
            )
            print(f"scheduler_type is {self.scheduler_type}")
            if self.scheduler_type in {"cosine_with_warmup", "constant_with_warmup", "poly_with_warmup"}:
                print(f"warmup steps is {int(self.max_steps * self.warmup)}")
        elif self.scheduler_type == "cosine_with_warmup":
            if self.warmup == 0.0:
                self.warmup = 0.1
        ...
        with tqdm(total=self.num_step_per_epoch, disable=(self.local_rank != 0)) as t:
            for i in range(self.num_step_per_epoch):
                ...
                if not self.auto_optim:
                    ...
                    with autocast("cuda", dtype=self.amp_dtype, enabled=self.amp_enabled) if (self.ddp or 'cuda' in self.device) else dummy_context():
                        if self.print_time:
                            s = time.time()
                        loss = self.training_step(batch)
                        if self.print_time:
