...
def _import_segmamba_aug():
    current_file = Path(__file__).resolve()
    brats23_root = current_file.parents[2]
    segmamba_root = brats23_root / "SegMamba"
    if str(segmamba_root) not in sys.path:
        sys.path.insert(0, str(segmamba_root))

    from light_training.augment.train_augment import get_train_transforms as seg_get_train_transforms
    from light_training.augment.train_augment import get_validation_transforms as seg_get_val_transforms
    return seg_get_train_transforms, seg_get_val_transforms


def _center_crop_3d(tensor, roi_size):
    ...
    eh = sh + min(rh, h)
    ew = sw + min(rw, w)
    return tensor[..., sd:ed, sh:eh, sw:ew]


class SegMambaStyleTransform:
    def __init__(self, crop_size, split="train"):
        seg_get_train_transforms, seg_get_val_transforms = _import_segmamba_aug()
        self.crop_size = tuple(crop_size)
        self.split = split
        self.transform = seg_get_train_transforms(self.crop_size) if split == "train" else seg_get_val_transforms()

        # SegMamba preprocess modality order: [t2w, t2f, t1n, t1c]
        self.idx_t2w = 0
        self.idx_t2f = 1
        self.idx_t1n = 2
        self.idx_t1c = 3
    ...
    def __call__(self, sample):
        transformed = self.transform({"data": sample["data"], "seg": sample["seg"]})

        data = self._to_tensor(transformed["data"], torch.float32)
        seg = self._to_tensor(transformed["seg"], torch.float32)
        mask = self._label_to_onehot(seg)

        out = {
            "t1n": data[self.idx_t1n],
            "t2w": data[self.idx_t2w],
            "t1c": data[self.idx_t1c],
            "t2f": data[self.idx_t2f],
            "mask": mask,
        ...
        if self.split != "train":
            out["t1n_crop"] = _center_crop_3d(out["t1n"], self.crop_size)
            out["t2w_crop"] = _center_crop_3d(out["t2w"], self.crop_size)
            out["t1c_crop"] = _center_crop_3d(out["t1c"], self.crop_size)
            out["t2f_crop"] = _center_crop_3d(out["t2f"], self.crop_size)
            out["mask_crop"] = _center_crop_3d(out["mask"], self.crop_size)

        return out


def get_train_transforms(crop_size):
    print("Using SegMamba-style train augmentation with crop_size", crop_size)
    return SegMambaStyleTransform(crop_size=crop_size, split="train")


def get_val_transforms(crop_size):
    return SegMambaStyleTransform(crop_size=crop_size, split="val")
