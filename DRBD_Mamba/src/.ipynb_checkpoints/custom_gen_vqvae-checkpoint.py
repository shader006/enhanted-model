...
class SegmentationModel(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, num_classes: int) -> None:
        ...
        self.conv3 = nn.Conv3d(4, 4, kernel_size=1)  # Output channels equal to num_classes
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # print("X shape is", x.shape)
        x = F.relu(x)
        segmentation_mask = self.conv3(x)
        print("X shape is", segmentation_mask.shape)
        
        # Compute softmax probabilities over classes
        output_probabilities = F.softmax(segmentation_mask, dim=1)
        
        # # Compute the maximum probability and its class
        # segmentation_mask = torch.argmax(output_probabilities, dim=1)
        # # result = [(segmentation_mask == 1) | (segmentation_mask == 3), (segmentation_mask == 1) | (segmentation_mask == 3) | (segmentation_mask == 2), (segmentation_mask == 3)]
        ...
        # final_segmentation_mask = torch.stack(result, dim=1).float()  # Shape: (batch_size, 3, H, W, D)
        # print("final segmentation", final_segmentation_mask.shape)
...
class VQVAE(nn.Module):
    ...
    def forward(self, images: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        return segmentation_mask, quantization_losses
