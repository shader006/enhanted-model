...
from src.cond_encoder_mri import cond_latent_encoder
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor | None = None) -> torch.Tensor:
        ...
        for block in self.transformer_blocks:
            # print("spatial context dim is", context.shape)
            # print("spatial x dim is", x.shape)
...
class CrossAttnDownBlock(nn.Module):
    ...
    def forward(
        ...
        output_states = []
        # if context != None:
            # print("My context shape is", context.shape)
        for resnet, attn in zip(self.resnets, self.attentions):
...
class CrossAttnMidBlock(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.resnet_1(hidden_states, temb)
        print("hidden_states", hidden_states.shape)
        print("attention context", context.shape)
...
class DiffusionModelUNet(nn.Module):
    ...
    #     self.cond_latent_encoder = cond_latent_encoder(
    # spatial_dims=3,  # 3D data
    # in_channels=4,  # Number of input channels (e.g., RGB or multi-modal data)
    # num_channels=[8, 16, 32, 64],  # Channels increase at each stage
    # num_res_channels=[8, 16, 32, 64],  # Residual channels matching the main channels
    # num_res_layers=1,  # Number of residual blocks at each stage
    # downsample_parameters=[(2, 3, 1), (2, 3, 1), (2, 3, 1), (1, 3, 1)],  # Downsampling parameters
    # dropout=0.3,  # Dropout rate, adjust as needed
    # act='relu',  # Activation function, can be 'relu', 'leaky_relu', etc.
    # ddp_sync=False,
    # )
        self.cond_latent_encoder = nn.Conv3d(512, 32, kernel_size=3, stride=1, padding=1)
    ...
    def forward(
        ...
        # print("My context shape is", context.shape)
        if context is not None:
            context = self.cond_latent_encoder(context)
            # print("My context updated shape is", context.shape)
            batch, channel, height, width, depth = context.shape
            context = context.view(batch, channel, height * width * depth).transpose(1, 2)
            # print("My context updatedddddddddddddddddd shape is", context.shape)
        ...
        for downsample_block in self.down_blocks:
            ...
            # print("size of input latent in LDM Unet", h.shape)
